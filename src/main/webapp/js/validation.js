    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.code !== 0) {
                ...
                Validation.renderCourses(data.object);
...
Validation.getEnrolledStudents = function getEnrolledStudents() {
    ...
    $.ajax({
        url: "/getEnrolledStudentsAdmin",
        type: 'POST',
        data: jsonParam,
        contentType: "application/json",
        success: function (data) {
            console.log(data.message);
            if (data.code !== 0 && data.object.length === 0) {
                validation.hide();
                $(warning2).find('h1').html(data.message);
                warning2.show();
            } else if (data.code !== 0 && data.object.length !== 0) {
                validation.hide();
                $(warning2).find('h1').html(data.message);
                warning2.show();
            } else {
                warning2.hide();
                Utils.sortStudentsAscending(data.object);
                Validation.renderStudents(data.object);
            }
        },
        error: function (data) {
            console.log(data);
...
Validation.enableValidationButtons = function enableValidationButtons() {
    ...
    var validateTooltip = $('#validateTooltip');
    var invalidateTooltip = $('#invalidateTooltip');

    validateButton.attr('disabled', false);
    invalidateButton.attr('disabled', false);

    validateTooltip.html(validateAll);
    invalidateTooltip.html(invalidateAll);
};
...
Validation.disableValidationButtons = function disableValidationButtons() {
    ...
    validateTooltip.html(nothingToValidate);
    invalidateTooltip.html(nothingToInvalidate);
};
